...
export type AlignmentType = 
    | 'left' | 'right' | 'center-x' | 'top' | 'bottom' | 'center-y'
    | 'baseline';

export type SpacingType = 'horizontal' | 'vertical';
...
export interface AlignmentConstraint {
    ...
    objects: string[]; // IDs of aligned objects
    tolerance: number; // Pixels of acceptable deviation (typically 2-5px)
    coordinate: number; // The alignment coordinate
}
...
export interface DistributionConstraint {
    ...
    objects: string[];
    spacing: number; // Equal spacing between objects
    tolerance: number;
}
...
export function detectConstraints(objects: CanvasObject[]): Constraint[] {
...
function detectAlignments(objects: CanvasObject[]): AlignmentConstraint[] {
...
function detectSpacing(objects: CanvasObject[]): SpacingConstraint[] {
    const constraints: SpacingConstraint[] = [];
    const TOLERANCE = 3;
...
function detectDistributions(objects: CanvasObject[]): DistributionConstraint[] {
    const constraints: DistributionConstraint[] = [];
    const TOLERANCE = 5;
...
function detectGrids(objects: CanvasObject[]): GridConstraint[] {
    const constraints: GridConstraint[] = [];
    const TOLERANCE = 5;
...
function groupByCoordinate(
    ...
    getCoord: (obj: CanvasObject) => number,
    tolerance: number
): CanvasObject[][] {
    const groups: CanvasObject[][] = [];
...
function groupObjectsIntoRows(objects: CanvasObject[], tolerance: number): CanvasObject[][] {
    const sortedByY = objects.slice().sort((a, b) => a.y - b.y);
...
function groupObjectsIntoColumns(objects: CanvasObject[], tolerance: number): CanvasObject[][] {
    const sortedByX = objects.slice().sort((a, b) => a.x - b.x);
...
function isAlignmentSatisfied(
    ...
    if (constraintObjects.length < 2) return true;
    
    const getAlignCoord = (obj: CanvasObject): number => {
...
function isDistributionSatisfied(
    ...
    if (constraintObjects.length < 3) return true;
...
export function computeConstraintDiff(
    ...
    const affectedObjects = new Set<string>([movedObjectId]);
...
function constraintInvolvesObject(constraint: Constraint, objectId: string): boolean {
    return getConstraintObjects(constraint).indexOf(objectId) !== -1;
}

function getConstraintObjects(constraint: Constraint): string[] {
    switch (constraint.type) {
        case 'alignment':
