...
// Call OpenAI API
function callOpenAI(apiKey_1, systemPrompt_1, userPrompt_1, onDebug_1, imageBase64_1, temperature_1) {
    return __awaiter(this, arguments, void 0, function* (apiKey, systemPrompt, userPrompt, onDebug, imageBase64, temperature, includeCoordinateContext = true) {
        var _a, _b, _c;
...
function saveGenerationLog(designPrompt, figmaDOM, frame, beforeScreenshot, afterScreenshot) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const loggerServerUrl = (yield figma.clientStorage.getAsync('logger-server-url')) || 'http://localhost:8000';
        // Send log to logger server
        try {
            const response = yield fetch(`${loggerServerUrl}/log`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
...
function findShapeByIdOrDescription(targetId, targetDescription, availableShapes) {
    if (targetId) {
...
function executeOperations(instructions, parentFrame, availableShapes) {
    const targetParent = parentFrame || figma.currentPage;
    const createdNodes = [];
    const modifiedNodes = [];
    for (const operation of instructions.operations) {
        try {
            if (operation.action === 'add') {
                ...
                switch (normalizedType) {
                    ...
                    case 'rectangle':
                        ...
                        if (operation.cornerRadius !== undefined) {
                            node.cornerRadius = operation.cornerRadius;
                        }
                        break;
                    case 'vector':
                        node = figma.createVector();
                    ...
                    case 'polygon':
                        ...
                        if (operation.pointCount !== undefined) {
                            node.pointCount = operation.pointCount;
                        }
                        if (operation.width !== undefined && operation.height !== undefined) {
                            node.resize(operation.width, operation.height);
                        }
                    ...
                    case 'star':
                        ...
                        if (operation.innerRadius !== undefined) {
                            node.innerRadius = operation.innerRadius;
                        }
                        if (operation.width !== undefined && operation.height !== undefined) {
                            node.resize(operation.width, operation.height);
                        }
                ...
                const targetNode = findShapeByIdOrDescription(operation.targetId, operation.targetDescription, availableShapes);
                if (!targetNode) {
...
function parseExecutionInstructions(response) {
    // Try to extract JSON from the response (it might be wrapped in markdown code blocks)
    let jsonString = response.trim();
    if (jsonString.startsWith('```json')) {
        jsonString = jsonString.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
    ...
    try {
        ...
        figma.ui.onmessage = (msg) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            if (msg.type === 'load-logger-url') {
                const loggerUrl = yield figma.clientStorage.getAsync('logger-server-url');
            ...
            if (msg.type === 'generate' && msg.prompt && msg.apiKey && msg.frameId) {
                try {
                    ...
                    const plan = yield callOpenAI(msg.apiKey, "You are a design planning assistant. Create detailed, precise plans for design modifications with exact numerical values.", planningPrompt, (request, response) => {
                        try {
                            const sanitizedRequest = JSON.parse(JSON.stringify(request));
                            const sanitizedResponse = JSON.parse(JSON.stringify(response));
                            figma.ui.postMessage({
                    ...
                    const executionResponse = yield callOpenAI(msg.apiKey, "You are a design execution assistant. Convert detailed plans into precise JSON instructions for Figma primitives.", executionPrompt, (request, response) => {
                        try {
                            const sanitizedRequest = JSON.parse(JSON.stringify(request));
                            const sanitizedResponse = JSON.parse(JSON.stringify(response));
                            figma.ui.postMessage({
                    ...
                    const affectedNodes = executeOperations(executionInstructions, currentFrame, currentVectors);
