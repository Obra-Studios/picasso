...
export interface Solution {
    ...
    score: number; // 0-100, quality of solution
...
export function quickSolve(
    ...
    if (isAlignmentOnly(constraints)) {
        return solveAlignments(constraints as AlignmentConstraint[], objectsMap, fixedSet);
    }
    
    // Try simple spacing solution
    if (isSimpleSpacing(constraints)) {
        return solveSpacing(constraints as SpacingConstraint[], objectsMap, fixedSet);
    }
    
    // Try distribution solution
    if (isSimpleDistribution(constraints)) {
        return solveDistribution(constraints as DistributionConstraint[], objectsMap, fixedSet);
    }
...
function isAlignmentOnly(constraints: Constraint[]): boolean {
    return constraints.length > 0 && constraints.every(c => c.type === 'alignment');
}

function isSimpleSpacing(constraints: Constraint[]): boolean {
    return constraints.length > 0 && constraints.every(c => c.type === 'spacing');
}

function isSimpleDistribution(constraints: Constraint[]): boolean {
...
function getAlignmentCoord(obj: CanvasObject, alignmentType: string): number {
...
function getAverageAlignmentCoord(objects: CanvasObject[], alignmentType: string): number {
    const coords = objects.map(obj => getAlignmentCoord(obj, alignmentType));
    return coords.reduce((a, b) => a + b, 0) / coords.length;
}

function applyAlignmentDelta(
    obj: CanvasObject,
...
function calculateScore(satisfied: number, unsatisfied: number, total: number): number {
